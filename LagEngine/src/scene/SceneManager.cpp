	...
	activeSceneName("")
{
	LogManager::getInstance().log(LAG_LOG_TYPE_INFO, LAG_LOG_VERBOSITY_NORMAL,
		"SceneManager", "Created successfully.");
}
...
void SceneManager::addRenderablesToQueue(RenderQueue &renderQueue, Viewport &viewport, RenderTarget &renderTarget) const
{
	if(hasActiveScene())
		...
		LogManager::getInstance().log(LAG_LOG_TYPE_ERROR, LAG_LOG_VERBOSITY_NORMAL,
			"SceneManager", "Trying to fill the Render Queue when there is not an active Scene.");
}
...
void SceneManager::addScene(const std::string &name, Scene &scene)
{
	...
	if (it != sceneMap.end())
	{
		LogManager::getInstance().log(LAG_LOG_TYPE_WARNING, LAG_LOG_VERBOSITY_NORMAL,
			"SceneManager", "Adding scene with already existent name: " + name + ". Replacing the older one.");
		removeScene(name);
	}

	LogManager::getInstance().log(LAG_LOG_TYPE_INFO, LAG_LOG_VERBOSITY_NORMAL,
		"SceneManager", "Added scene with name: " + name + ".");
	sceneMap[name] = &scene;

	if (!hasActiveScene())
	{
		LogManager::getInstance().log(LAG_LOG_TYPE_INFO, LAG_LOG_VERBOSITY_NORMAL,
			"SceneManager", "Auto-setting the Scene " + name + " as the active Scene.");
		setActiveScene(name);
	}
}
...
void SceneManager::removeScene(const std::string &name)
{
	...
	{
		LogManager::getInstance().log(LAG_LOG_TYPE_INFO, LAG_LOG_VERBOSITY_NORMAL,
			"SceneManager", "Removed scene with name: " + name + ".");
		delete it->second;
		sceneMap.erase(it);
	}
	else
	{
		LogManager::getInstance().log(LAG_LOG_TYPE_WARNING, LAG_LOG_VERBOSITY_NORMAL,
			"SceneManager", "Trying to remove a non-existent Scene: " + name  + ".");
	}

	if (activeSceneName == name)
	{
		LogManager::getInstance().log(LAG_LOG_TYPE_WARNING, LAG_LOG_VERBOSITY_NORMAL,
			"SceneManager", "Removing the active Scene: " + name + ".");
		activeSceneName = "";
	}
}
...
{ 
	if(!hasActiveScene())
		LogManager::getInstance().log(LAG_LOG_TYPE_ERROR, LAG_LOG_VERBOSITY_NORMAL,
			"SceneManager", "Trying to get the active Scene, but none is set.");
...
void SceneManager::clear()
{
	...
	LogManager::getInstance().log(LAG_LOG_TYPE_INFO, LAG_LOG_VERBOSITY_NORMAL,
		"SceneManager", "Cleared " + std::to_string(sceneMap.size()) + " Scenes.");
...
void SceneManager::setActiveScene(const std::string &name)
{
	...
	{
		LogManager::getInstance().log(LAG_LOG_TYPE_ERROR, LAG_LOG_VERBOSITY_NORMAL,
			"SceneManager", "Trying to set a non-existent Scene as active: " + name + ".");
		return;
	}
	...
	{
		LogManager::getInstance().log(LAG_LOG_TYPE_WARNING, LAG_LOG_VERBOSITY_NORMAL,
			"SceneManager", "Setting the same active Scene as the currently active: " + name + ". Ignoring.");
		return;
	}
